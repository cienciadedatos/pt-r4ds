...
Começaremos com as funções que operam sobre as linhas e colunas de um data frame, em seguida voltaremos para falar mais um pouco sobre o *pipe*, que é uma importante ferramenta que você utilizará para combinar os verbos[^data-transform-1] das transformações.
...
[^data-transform-1]: **Nota de tradução**: No pacote dplyr, chamamos as principais funções de "verbos" para enfatizar que elas fazem uma ação.
    Os principais verbos do dplyr são: `filter()` (filtrar), `arrange()` (ordenar), `select()` (selecionar), `mutate()` (alterar), `summarize()` (sumarizar), e `group_by()` (agrupar).
...
Em ambas as visualizações, o nome das variáveis estão seguidos por abreviaturas que te indicam o tipo de cada variável: `<int>` para inteiros, `<dbl>` double[^data-transform-2] (números reais), `<chr>` para caracteres (strings) e `<dttm>` para date-time (data e hora).
Isso é importante pois as operações possíveis sobre uma coluna depende muito do seu "tipo".

[^data-transform-2]: de *double precision floating number* - remete à forma como números reais são representados nos computadores, utilizando 64 bits em vez dos tradicionais 32 bits de um inteiro comum, por isso "double" (dobro).
...
`filter()` lhe permite manter linhas com base nos valores das colunas[^data-transform-3].
...
[^data-transform-3]: Depos você irá aprender sobre a família de funções `slice_*()` que permite escolher linhas com base em suas posições.
...
1.  Utilizando um único pipeline para cada condição, ache todos os vôos que cumpram cada condição:
    ...
    -   Decolou no verão[^data-transform-4] (julho, agosto e setembro)
...
[^data-transform-4]: no hemisfério norte
...
Podemos usar o argumento `.before` para, em vez disso, adicionar as variáveis à esquerda[^data-transform-5]:

[^data-transform-5]: Lembre-se que no RStudio, o jeito mais fácil de visualizar um conjunto de dados com muitas colunas é `View()`.
...
No dplyr, essa operação é feita pela função `summarize()`[^data-transform-6], como mostrado pelo exemplo a seguir, que calcula o atraso médio das decolagens por mês:

[^data-transform-6]: Ou `summarise()`, se você preferir o inglês britânico.
...
Quando montamos um gráfico da habilidade dos rebatedores (medidas pela média de acerto, `performance`) contra o número de oportunidades de acertar a bola (medidas pela número de vezes ao bastão, `n`), é possível ver dois padrões:

1.  A variação em `performance` é maior entre os jogadores que foram menos vezes ao bastão.
    A forma do gráfico é muito característica: sempre que você projetar no gráfico uma média (ou alguma outra sumarização estatística) vs. tamanho do grupo, você verá que a variação diminui à medida que o tamanho da amostra aumenta[^data-transform-7].
...
[^data-transform-7]: \*cof-cof\* a lei dos grandes números \*cof-cof\*.
