...
Os princípios do formato *tidy* parecem ser tão óbvios que você deve estar se perguntando se algum dia vai encontrar algum conjunto de dados que não esteja nesse formato.
...
Suponha que temos três pacientes com `id`s A, B e C, e fazemos a medição da pressão sanguínea (`ps`) duas vezes para cada paciente.
...
knitr::include_graphics("diagrams/tidy-data/variables-pt.png", dpi = 270)
```
...
knitr::include_graphics("diagrams/tidy-data/column-names-pt.png", dpi = 270)
```
...
knitr::include_graphics("diagrams/tidy-data/cell-values-pt.png", dpi = 270)
```
...
Por exemplo, pegue o conjunto de dados `dados_oms2`, que é a fonte da `tabela1` e outras similares que você viu acima:
...
Existem duas colunas que já são variáveis e parecem fáceis de interpretar: `pais` e `ano`. Em seguida vêm outras 56 colunas tais como `fpp_m_014`, `fpn_m_4554` e `ep_m_3544`. Se você olhar com cuidado para essas colunas por tempo suficiente, verá que há um padrão.
...
[^data-tidy-3]: **Nota de tradução**: do inglês *World Health Organisation* (WHO).

Então, nesse caso temos seis partes de informação registrados em `dados_oms2`: o país e o ano (já nas colunas); o método diagnóstico, a categoria do gênero, e a categoria do intervalo de idade (contido nos outros nomes de colunas); e a contagem dos pacientes em naquela categoria (valores das células).
...
knitr::include_graphics("diagrams/tidy-data/multiple-names-pt.png", dpi = 270)
```
...
knitr::include_graphics("diagrams/tidy-data/names-and-values-pt.png", dpi = 270)
```
...
Os exemplos apresentados aqui são uma seleção dos vários presentes em `vignette("pivot", package = "tidyr")`, então, se você encontrar algum problema com o qual esse capítulo ainda não é capaz de te ajudar, é uma boa ideia checar nessa *vignette*[^data-tidy-4].

[^data-tidy-4]: **Nota de tradução:** As *vignettes* são documentos que acompanham os pacotes do R e fornecem documentações mais extensas sobre o pacote e suas funções.
