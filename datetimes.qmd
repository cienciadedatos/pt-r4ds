...
-   Uma **data-horário** é uma data mais um horário: identifica exclusivamente um instante no tempo (tipicamente até o segundo mais próximo).
...
Datas-horário são substancialmente mais complicadas devido à necessidade de lidar com fusos horários, o que abordaremos no final do capítulo.
...
-   Durante a leitura de um arquivo com readr.
...
`ymd()` e amigos criam datas.
Para criar uma data-horário, adicione um sublinhado e uma ou mais de "h", "m" e "s" ao nome da função de análise:
...
Você também pode forçar a criação de uma data-horário a partir de uma data fornecendo um fuso horário:
...
#|   nos fins de semana. São poucos os dias que se destinoacam por terem tido surpreendentemente
...
Observe que quando você usa datas-horários em um contexto numérico (como em um histograma), 1 significa 1 segundo, então uma largura de bin de 86400 significa um dia.
...
Alternativamente, em vez de modificar uma variável existente, você pode criar uma nova data-horário com `update()`.
...
(Se o R não souber, você receberá um `NA`.) E veja a lista completa de todos os nomes de fusos horários com `OlsonNames()`:
...
Este capítulo apresentou as ferramentas que o lubridate fornece para ajudá-lo a trabalhar com dados de data e hora.
Trabalhar com datas e horários pode parecer mais difícil do que o necessário, mas espero que este capítulo tenha ajudado você a entender o porquê --- datas e horários são mais complexos do que parecem à primeira vista, e lidar com todas as situações possíveis adiciona complexidade.
...
O próximo capítulo faz um resumo dos valores ausentes.
