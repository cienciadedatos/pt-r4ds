...
Isto acontece porque as expressões regulares consideram a diferença entre letras maiúsculas e minúsculas (*case sensitive*)[^regexps-5].

[^regexps-5]: **Nota de tradução**: Dizemos que a linguagem de programação é *case sensitive* quando existe a diferença entre letras maiúsculas (*upper case*) e minúsculas (*lower case*).
...
Aqui temos alguns dados derivados de `dados::bebes` onde temos nome, sexo biológico e idade de várias pessoas em um formato meio estranho[^regexps-6]:

[^regexps-6]: Gostaríamos de poder garantir que você nunca veria algo tão estranho na vida real, mas infelizmente, ao longo de sua carreira, é provável que você veja coisas muito mais estranhas!
...
Para identificar um `.` literal, você precisa **escapar**, o que informa a expressão regular para interpretar o metacaractere [^regexps-7] literalmente.
...
[^regexps-7]: A lista completa de metacaracteres é `.^$\|*+?{}[]()`
...
Existem três outras classes que são particularmente úteis[^regexps-8]:

[^regexps-8]: Lembre-se, para criar uma expressão regular contendo `\d` ou `\s`, você precisará escapar a `\` da *string*, então você digitará `"\\d"` ou `"\\s"`.
...
Mas `str_match()` retorna uma matriz, então não é particularmente fácil trabalhar com ela[^regexps-9]:

[^regexps-9]: Principalmente porque nunca discutimos sobre matrizes neste livro!
...
Isso permite que você use comentários e espaços em branco para tornar expressões regulares complexas mais compreensíveis[^regexps-10], como no exemplo a seguir:

[^regexps-10]: `comments = TRUE` é particularmente eficaz quando usado com uma string bruta (*raw*), como usamos aqui.
